...
public class Solution {
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        if(image[sr][sc]!=newColor)
            fillColor(image, sr, sc, newColor, image[sr][sc], image.length, image[0].length);
        return image;
    }

    public void fillColor(int[][] image, int sr, int sc, int newColor, int oldColor, int m, int n) {
        image[sr][sc] = newColor;
        if (sr - 1 >= 0 && image[sr - 1][sc] == oldColor) {
            fillColor(image, sr - 1, sc, newColor, oldColor, m, n);
        }
        if (sr + 1 < m && image[sr + 1][sc] == oldColor) {
            fillColor(image, sr + 1, sc, newColor, oldColor, m, n);
        }
        if (sc - 1 >= 0 && image[sr][sc - 1] == oldColor) {
            fillColor(image, sr, sc - 1, newColor, oldColor, m, n);
        }
